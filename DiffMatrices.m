function [R, Fb_layer, BB1, BB2, BB3, LL, UU] = DiffMatrices(R, Fdiv)
...
% full of zeros
LL = sparse(aux2, aux2); UU = sparse(aux2, aux2); BB1 = sparse(aux2, aux2); BB2 = sparse(aux2, aux2);  BB3 = sparse(aux2, aux2);
[I,J] = ind2sub([aux, aux],1:aux*aux);
for k = 1:St.numStVLiq2
    aa = a(ind(k,1):ind(k,2),ind(k,1):ind(k,2));
    A = D - aa.*Lxy; % the term at t+1
    BB1i = D + aa.*Lxy; % the term at t
    B2 = 2*aa.*Bc; % the coefficients for boundary conditions of Dirichlet type
    B3 = sparse(1:aux,1:aux,ones(Sxy.nT,1),aux,aux); % identity sparse matrix
    [L,Ui] = lu(A); % perform LU decomposition
    LL = LL + sparse((k-1)*aux+I,(k-1)*aux+J,reshape(L, [],1),aux2,aux2); % Lower matrix for all liquid species 
    UU = UU + sparse((k-1)*aux+I,(k-1)*aux+J,reshape(Ui, [],1),aux2,aux2); % Upper matrix for all liquid species
    BB1 = BB1 + sparse((k-1)*aux+I,(k-1)*aux+J,reshape(BB1i, [],1),aux2,aux2);
