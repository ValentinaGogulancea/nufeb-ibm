...
PathOutput = ['.\' datestr(now,'dd-mmm-yyyy HH-MM') '\'];
mkdir(PathOutput)
[SUCCESS,MESSAGE,MESSAGEID] = copyfile( 'R.mat', PathOutput,'f');

[R, ~, A, BB2, BB3] = DiffMatricesEuler(R, 1); % ccommented further inside
R = boundary(R, dT, 1); % solves the reactor mass balance and assigns new boundary conditions
Sbc_Dir = kron(R.Sxy.Sbc_Dir(1:numStVLiq2), ones(R.Sxy.nT,1)); % concentrations in mol/L for the boundary at the top of the biofilm

NRVliq = R.rm.NRVliq;
U = R.Sxy.StVLiq2 ; % concentrations of liquid species - mol/L
...
out_integTime(0, R, 0,PathOutput); % see comments there
...
while i< nTBac
    ...
    j = 1;
    tol = 1e-8;
    % the dt no longer exists here
    fx = A * U + BB2 * Sbc_Dir + BB3*NRVliq;
    while ~all(abs(fx(:)) < tol)
        dU = -(A + diag(dNRVliq)) \ fx;
        U = U + dU;
        fx = A * U + BB2 * Sbc_Dir + NRVliq;
        [~, NRV, dNRV, ~, ~, ~, ~] = my_kinetics(R);  % update the rates
        NRVliq = NRV(1:numSt);
        dNRVliq = dNRV(1:numSt);
%         max(abs(fx(:))./U)
        j = j + 1;
...
j
    ...
    R.rm.NRVbac = NRVbac;
    V = V + j * dT*NRVgas;  R.Sxy.StVGas = V;
    W = W + j * dT*NRVbac; R.bac.bac_m = W; % updating biomass concentration
    R.Sxy.StVLiq = [U; V];
    ...
    %     SS = 2 * Tol;
    %     dT_bac = dT_bac - j * dT;
    R = massBal( U, V, W, R); % update the reaction matrices - for next diffusion step
    ...
    if mod(i,dT_Print/dT_bac) == 0
        [R, Fdiv] = bacteria( U, V, W, R); % this calls the shoving algorithm
        [R, Fb_layer, A, BB2, BB3] = DiffMatricesEuler(R, Fdiv); % computing the matrices again
        fprintf('Number of bacteria: %.0f', R.bac.bac_n)
        fprintf('\n')
        fprintf('Mass of bacteria: %.6e', sum(R.bac.bac_m)*24.6)
        save('R.mat', 'R', '-v7.3');
        out_integTime(dT_bac*i, R, j == nTDiff,PathOutput);
        %
        if Fdiv == 1
    ...
    Sbc_Dir = kron(R.Sxy.Sbc_Dir(1:numStVLiq2), ones(R.Sxy.nT,1));
    fprintf('\n')
    i = i + 1;
end
end
